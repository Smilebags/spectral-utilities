...
const scopeContext = scopeCanvas.getContext("2d", options);
const colourContext = colourCanvas.getContext("2d", options);
...
function render(scopeCtx, colourCtx, a, b, c) {
...
function renderColourScope(ctx, spectrum) {
    ...
    if (!isInGamut) {
        if (USE_P3_IMAGEDATA_HACK) {
            drawP3RectUsingImageData(ctx, ctx.canvas.width - 40, ctx.canvas.height - 40, 40, 40, new Colour(new Vec3(1, 0, 0), 'Display-P3'));
        }
        else {
...
function renderScopeBackground(ctx) {
    let imageData = null;
    if (USE_P3_IMAGEDATA_HACK) {
...
function assignRectToP3ImageData(imageData, xCoord, yCoord, width, height, colour) {
...
function drawP3RectUsingImageData(ctx, x, y, width, height, colour) {
