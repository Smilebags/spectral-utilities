...
const midEl = document.querySelector('#mid') as HTMLInputElement;
const widthEl = document.querySelector('#width') as HTMLInputElement;
const sharpnessEl = document.querySelector('#sharpness') as HTMLInputElement;
...
const createSliderEventHandler = (propName: keyof typeof state) => {
  return (e: Event) => {
    if (!e) {
      return;
    }
    state[propName] = Number((e.target as HTMLInputElement).value);
    render(true);
  }
}
...
function arrayFromRange(length: number, low: number, high: number): number[] {
  const range = high - low;
  const progress = (val: number) => val / (length - 1);
