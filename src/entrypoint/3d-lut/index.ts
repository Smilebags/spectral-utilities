...
function render(sum: number, clear = false) {
  if (clear) {
  ...
  const colours = results.map(result => Colour.fromSpectrum(
    result.spectrum,
    SPECTRUM_SAMPLING_PRECISION,
    WAVELENGTH_LOW,
    WAVELENGTH_HIGH,
  ));
...
function xyzSum(spectrum: Spectrum): number {
